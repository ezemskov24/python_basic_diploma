...
def get_city(message: Message) -> None:
    if message.text.isalpha():
        ...
        bot.set_state(message.from_user.id, FindInfoState.min_price, message.chat.id)
    else:
        bot.send_message(message.from_user.id, "Что-то пошло не так, попробуйте ввести еще раз")


@bot.callback_query_handler(func=lambda call: True)
def hotel_in_city(call):
    pass


@bot.message_handler(state=FindInfoState.min_price)
def min_price(message: Message) -> None:
    bot.send_message(message.from_user.id, "Введите минимальную цену за ночь")
    bot.set_state(message.from_user.id, FindInfoState.max_price, message.chat.id)
